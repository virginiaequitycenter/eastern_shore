...
ui <- dashboardPage(skin = "purple",
  dashboardHeader(title = "Eastern Shore Profile", titleWidth = 400),
  ...
  dashboardBody(
              ...
              fluidRow(
                ...
                # Sidebar layout
                box(tags$h3("Social/Demographic Indicators"),
...
server <- function(input, output, session) {
  ...
  output$map <- renderLeaflet({
    leaflet() %>%
      ...
      addPolygons(data = subset(data_geo(), county.nice %in% input$geo),
                  fillColor = ~pal()(d1()),
                  fillOpacity = 0.5,
                  color = "white",
                  weight = 2,
                  ...
                  popup = paste0(input$indicator, ": ",d1(),  "<br>",
                                 data_geo()$NAME[data_geo()$county.nice %in% input$geo], "<br>"
                  ),
      ...
      addPolygons(data = mag(), group="Magesterial Districts",
                  ...
                  highlight = highlightOptions(weight = 3,
                                               ...
                                               bringToFront = TRUE)) %>% 
      addLayersControl(
        overlayGroups = c("Schools", "Elem School Zone", "Magesterial Districts"),
  ...
  # output data table ----
  output$tbl <-  renderDT({
    datatable(subset(df(), county.nice %in% input$geo),
  ...
  output$hist <- renderPlotly({ 
    ...
    if (!input$indicator2=="None") {
      ...
      plot_ly(data=df(),  
              x=df()[df()$county.nice %in% input$geo,  
                     paste(prettytab()[prettytab()$goodname==input$indicator, "varname"])],
              type="histogram", marker = list(color = '#31688EFF')) %>%
        layout(xaxis=list(title=input$indicator))
    }
  })
    ...
    if (input$indicator %in% c("Median Household Income", "Life Expectancy at Birth") | input$indicator2 %in% c("Median Household Income", "Life Expectancy at Birth")) {
      ggplotly(
        ggplot(subset(race_comp, NAME %in% input$geo & var == race_var()),
          ...
          scale_fill_manual(values=mycolors[c(2,4,6,8,10)]) +
