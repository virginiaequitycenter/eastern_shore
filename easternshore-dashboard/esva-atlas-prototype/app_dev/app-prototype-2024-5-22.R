...
pop_est <- pop %>% 
  select(GEOID, tract_id, totpop_est,
         ...
         age17per_est, age65per_est, percent_lowage_h, percent_lowage_w,
         bldgage70per_est
         ) %>% 
  mutate(GEOID = as.character(GEOID),
         ...
         percent_lowage_h = round(percent_lowage_h,0),
         percent_lowage_w = round(percent_lowage_w,0),
         bldgage70per_est = round(bldgage70per_est),0)
...
names(storm_dat) = c(
  ...
  "remainper_est", "Population under 18 yrs", "Population over 65 yrs", 
  "Homes of low-wage workers", "Low-wage Jobs", "Housing built before 1970",
  "geometry"
)
      ...
      sidebar = sidebar(
        selectInput('pop_name', 
                    ...
                    choices = c("Percent Black Population", "Percent Hispanic Population", "Percent White Population",
                                "Population under 18 yrs", "Population over 65 yrs", "Homes of low-wage workers", 
                                "Low-wage Jobs", "Housing built before 1970"), 
...
server <- function(input, output, session){
  ...
  output$map <- renderLeaflet({
    ...
    m <- m %>% 
      mutate(label = paste0('Census Block: ', names, '<br/>',input$variable, ": ",round(m[[input$variable]], digits = 2)))
  ...
  output$heatmap <- renderHighchart({
    ...
    heat_dat <- if(sel == "Hurricane Isabel 2050 Projection"){
      heat_dat %>% filter(group == "Scenario2050")
    } else if(sel == "Hurricane Isabel 2003"){
  ...
  output$scatter <- renderHighchart({
    ...
    chart <- highchart() %>% 
        ...
        list(
          ...
          labels = list(
            ...
            list(
              ...
              align = 'right',
              point = list(x = 92, y = ymean, xAxis= 0, yAxis= 0),
      ...
      hc_tooltip(
        formatter = JS("function(){
                            return ('<strong>Census Block:</strong> ' + this.point.names +  ' <br>' + this.point.ylabel + ': ' + this.y + ' <br>' + this.point.xname + ': '  + this.x)
                            }")
      ) %>% 
      hc_plotOptions(
