...
storm_blkgrp <- storm_surge %>% mutate(GEOID = as.character(GEOID))

king_tide <- read_delim("KingTide_BLGP.txt")
king_tide <- king_tide %>% 
  mutate(GEOID = as.character(GEOID)) %>% 
  rename("PeakSurge_KT" = "PeakSurge",
         "MeanSurge_KT" = "MeanSurge",
         "Frac_KT" = "Frac")

king_tide_2050 <- read_delim("KingTide_2050_BLGP.txt")
...
blkgrp_geo <- st_read("cbg-selected/esva_2020blkgrp_clipped.geojson")
...
storm_2050_dat <- storm_2050_dat %>% 
  left_join(king_tide_2050)
      ...
      layout_sidebar(
        ...
        sidebar = sidebar(
          ...
          prettyRadioButtons(
            ...
            shape = c("round"),
            width = NULL,
            choices = c("Peak Surge", "Mean Surge", "Inundation Area Fraction","PeakSurge_KT", "MeanSurge_KT", "Frac_KT")
          ),
          # radioButtons(
          #   "variable",
          #   "Climate Risk:",
  ...
  output$map <- renderLeaflet({
    ...
    map <- leaflet(m) %>%
      ...
      addLegend(position = 'bottomright', pal = pal_rev,
                ...
                # }
                labFormat = labelFormat(transform = function(x) sort(round(x, 1), decreasing = TRUE), suffix = " m")
