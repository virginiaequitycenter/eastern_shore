...
es_blkgrp_pop <- sf::st_as_sf(es_blkgrp_pop)

 PeakSurge_m <- es_blkgrp_pop %>% 
  mutate(entropy = PeakSurge_m,
         group = "StormSurge") 
  
 MeanSurge_m <- es_blkgrp_pop %>% 
  mutate(entropy = MeanSurge_m,
         group = "StormSurge") 

 InundationAreaFraction <- es_blkgrp_pop %>% 
  mutate(entropy = InundationAreaFraction,
         group = "StormSurge") 

 PeakSurge2050_m <- es_blkgrp_pop %>% 
  mutate(entropy = PeakSurge2050_m,
         group = "StormSurge2050") 

 MeanSurge2050_m <- es_blkgrp_pop %>% 
  mutate(entropy = MeanSurge2050_m,
         group = "StormSurge2050") 
 
 InundationAreaFraction2050 <- es_blkgrp_pop %>% 
   mutate(entropy = InundationAreaFraction2050,
          group = "StormSurge2050") 

data_test <- list(PeakSurge_m=PeakSurge_m, MeanSurge_m=MeanSurge_m, InundationAreaFraction=InundationAreaFraction, PeakSurge2050_m=PeakSurge2050_m, MeanSurge2050_m=MeanSurge2050_m, InundationAreaFraction2050=InundationAreaFraction2050)
...
storm_blkgrp_long <- storm_blkgrp %>%
  ...
  mutate(group = case_when(str_detect(variable, "2050") ~ "StormSurge2050",
                           .default = "StormSurge"),
    # variable = str_remove(variable, "per_sum_"),
         variable = factor(variable, levels = c("var_PeakSurge_m", "var_MeanSurge_m", "var_InundationAreaFraction", "var_PeakSurge2050_m", "var_MeanSurge2050_m", "var_InundationAreaFraction2050"),
                           labels = c("PeakSurge_m", "MeanSurge_m", "InundationAreaFraction", "PeakSurge2050_m", "MeanSurge2050_m", "InundationAreaFraction2050")
  ...
  fluidRow(
    column(2, 
           radioButtons(
             "metro_name", 
             "Storm Surge", 
             choices = c("PeakSurge_m", "MeanSurge_m", "InundationAreaFraction", "PeakSurge2050_m", "MeanSurge2050_m", "InundationAreaFraction2050"), 
    ...
    column(10,
           fluidRow(
             ...
             fluidRow(
               column(3, highchartOutput('heatmap', height = '650px')),
               column(6,leafletOutput('map', height = '650px')),
               column(3, "")
               )
             )
           )
...
server <- function(input, output, session){
  ...
  output$heatmap <- renderHighchart({
    ...
    vars <- storm_blkgrp_long %>%
      group_by(variable, names)

    vars <- if(str_detect(sel, "2050")){
         vars %>% filter(group == "StormSurge2050")
       } else { vars }
