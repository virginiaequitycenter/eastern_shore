---
title: "What topics do you want the Climate Equity Atlas to include?"
# date: "2024-01-18"
...
```{r, warning=FALSE, echo=FALSE, message=FALSE, fig.width = 9, fig.height=8}
# By Age ----
...
plot_blkgrp_age <- 
  blkgrp_age %>% 
  group_by(county) %>%
  ggplot(aes(x = county, y = percent, color = county)) +
  geom_point(size=7, alpha = 0.2, position=position_dodge(preserve = "single")) +
  geom_point(data = county_age, aes(x = county, y = percent, color = county), shape = 15, size = 9, alpha = .9, position=position_dodge(preserve = "single")) +
  scale_y_continuous(limits = c(0, 100),
                     labels = scales::percent_format(scale = 1),
                           name = "Percent of Population") +
  scale_x_discrete(name = "",
  ...
  #   arrow = arrow(length = unit(0.5, 'cm'))
  # ) +
  facet_grid(~groups, switch = "x", scales = "free_x", space = "free_x") +
  theme_minimal() +
  theme(legend.title=element_blank(),
        legend.text = element_text(size = 12),
...
annotate_figure(
  ggarrange(plot_blkgrp_age, ncol = 1, nrow = 1, common.legend = TRUE),
  top = text_grob("Residents Who are Under 18, 18-64 years old, and 65 and Older", face = "bold", size = 16)
)
...
plot_blkgrp_race <- 
  ...
  ggplot(aes(x = county, y = percent, color = county)) +
  geom_point(size=7, alpha = 0.2, position=position_dodge(preserve = "single")) +
  geom_point(data = county_race, aes(x = county, y = percent, color = county), shape = 15, size = 9, alpha = .9, position=position_dodge(preserve = "single")) +
  scale_y_continuous(limits = c(0, 100),
                     labels = scales::percent_format(scale = 1),
                           name = "Percent of Population") +
  scale_x_discrete(name = "",
...
annotate_figure(
  ggarrange(plot_blkgrp_race, ncol = 1, nrow = 1, common.legend = TRUE),
  top = text_grob("Residents Who Identify as White, Black and Hispanic", face = "bold", size = 16)
)
...
```{r, warning=FALSE, echo=FALSE, message=FALSE, fig.width = 7, fig.height=8}
# By Jobs - low income employment ----
county_jobs <- popdat_county %>% 
  filter(demographic == "jobs") %>% 
  mutate(percent = (estimate/total_jobs) * 100,
         groups = factor(variable, levels = c("jobs_lowage_w", "jobs_midwage_w", "jobs_hiwage_w", "jobs_lowage_h", "jobs_midwage_h", "jobs_hiwage_h"), labels = c("Location of Job \nfor Low-wage Workers", "Mid-wage, Work", "High-wage, Work", "Location of Home \nfor Low-wage Workers", "Mid-wage, Home", "High-wage, Home"))) %>% 
...
blkgrp_jobs_pop <- popdat_blkgrp %>% 
  ...
  mutate(groups = factor(groups, levels = c("jobs_lowage_w", "jobs_lowage_h"), labels = c("Location of Job \nfor Low-wage Workers", "Location of Home \nfor Low-wage Workers"))) %>%
  filter(totpop_est != 0) %>% 
  na.omit()
...
blkgrp_jobs_per <- popdat_blkgrp %>% 
  pivot_longer(cols = c("percent_lowage_w", "percent_lowage_h"),
               ...
               values_to = "percent") %>%
  select(c("block_grp", "tract", "county", "groups", "percent")) %>%
  mutate(groups = factor(groups, levels = c("percent_lowage_w", "percent_lowage_h"), labels = c("Location of Job \nfor Low-wage Workers", "Location of Home \nfor Low-wage Workers"))) %>%
  na.omit()
...
plot_blkgrp_jobs <- 
  ...
  ggplot(aes(x = county, y = percent, color = county)) +
  geom_point(size=7, alpha = 0.2, position=position_dodge(preserve = "single")) +
  geom_point(data = county_jobs, aes(x = county, y = percent, color = county), shape = 15, size = 9, alpha = .9, position=position_dodge(preserve = "single")) +
  scale_y_continuous(limits = c(0, 100),
                     labels = scales::percent_format(scale = 1),
                           name = "Percent of Population") +
  scale_x_discrete(name = "",
...
annotate_figure(
  ggarrange(plot_blkgrp_jobs, ncol = 1, nrow = 1, common.legend = TRUE),
  top = text_grob("Workers in Jobs Paying Less Than $15,000/year, \n by Where They Work and Where They Live", face = "bold", size = 16)
)
...
```{r, warning=FALSE, echo=FALSE, message=FALSE, fig.width = 6, fig.height=8}
# By Income ----
...
plot_blkgrp_income <- 
  ...
  geom_point(size=7, alpha = 0.2, position=position_dodge(width = .75)) +
  geom_point(data = county_income, aes(x = county, y = estimate, color = county), shape = 15, size = 9, alpha = .9, position=position_dodge(width = .75)) +
  scale_y_continuous(limits = c(0, 100000),
                     labels = scales::dollar_format(prefix="$"),
                           name = "Median Household Income") +
  scale_x_discrete(name = "") +
...
annotate_figure(
  ggarrange(plot_blkgrp_income, ncol = 1, nrow = 1, common.legend = TRUE),
  top = text_grob("Median Household Income \n(Half of Households Make More, \nHalf of Households Make Less)", face = "bold", size = 16)
)
...
plot_blkgrp_tenure <- 
  ...
  ggplot(aes(x = county, y = percent, color = county)) +
  geom_point(size=7, alpha = 0.2, position=position_dodge(preserve = "single")) +
  geom_point(data = county_tenure, aes(x = county, y = percent, color = county), shape = 15, size = 9, alpha = .9, position=position_dodge(preserve = "single")) +
  scale_y_continuous(limits = c(0, 100),
                     labels = scales::percent_format(scale = 1),
                           name = "Percent of Households") +
  scale_x_discrete(name = "",
...
annotate_figure(
  ggarrange(plot_blkgrp_tenure, ncol = 1, nrow = 1, common.legend = TRUE),
  top = text_grob("Homes Owned and Rented", face = "bold", size = 16)
)
...
plot_blkgrp_hvalues <- 
  ...
  geom_point(size=7, alpha = 0.2, position=position_dodge(width = .75)) +
  geom_point(data = county_hvalues, aes(x = county, y = estimate, color = county), shape = 15, size = 9, alpha = .9, position=position_dodge(width = .75)) +
  scale_y_continuous(limits = c(0, 550000),
                     breaks = c(0, 100000, 200000, 300000, 400000, 500000),
                     labels = scales::dollar_format(prefix="$"),
                           name = "Median Housing Value") +
...
annotate_figure(
  ggarrange(plot_blkgrp_hvalues, ncol = 1, nrow = 1, common.legend = TRUE),
...
plot_blkgrp_rents <- 
  ...
  geom_point(size=7, alpha = 0.2, position=position_dodge(width = .75)) +
  geom_point(data = county_rents, aes(x = county, y = estimate, color = county), shape = 15, size = 9, alpha = .9, position=position_dodge(width = .75)) +
  scale_y_continuous(limits = c(0, 2000),
                     labels = scales::dollar_format(prefix="$"),
                           name = "Median Rent") +
  scale_x_discrete(name = "") +
...
annotate_figure(
  ggarrange(plot_blkgrp_rents, ncol = 1, nrow = 1, common.legend = TRUE),
...
plot_blkgrp_online <- 
  ...
  ggplot(aes(x = county, y = percent, color = county)) +
  geom_point(size=7, alpha = 0.2, position=position_dodge(preserve = "single")) +
  geom_point(data = county_online, aes(x = county, y = percent, color = county), shape = 15, size = 9, alpha = .9, position=position_dodge(preserve = "single")) +
  scale_y_continuous(limits = c(0, 100),
                     labels = scales::percent_format(scale = 1),
                           name = "Percent of Households") +
  scale_x_discrete(name = "",
...
annotate_figure(
  ggarrange(plot_blkgrp_online, ncol = 1, nrow = 1, common.legend = TRUE),
  top = text_grob("Residents with Both a Computer \nand Broadband Internet at Home", face = "bold", size = 16)
)
...
plot_blkgrp_hage <- 
  ...
  ggplot(aes(x = county, y = percent, color = county)) +
  geom_point(size=7, alpha = 0.2, position=position_dodge(preserve = "single")) +
  geom_point(data = county_hage, aes(x = county, y = percent, color = county), shape = 15, size = 9, alpha = .9, position=position_dodge(preserve = "single")) +
  scale_y_continuous(limits = c(0, 100),
                     labels = scales::percent_format(scale = 1),
                           name = "Percent of Housing Units") +
  scale_x_discrete(name = "",
...
annotate_figure(
  ggarrange(plot_blkgrp_hage, ncol = 1, nrow = 1, common.legend = TRUE),
  top = text_grob("Housing Units Built Before and After 1970", face = "bold", size = 16)
)
