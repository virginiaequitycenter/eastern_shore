...
pop <- box_read_csv("1883093293417") %>% 
...
group_vars <- syms(gw_df %>% select(starts_with("bin")) %>% names())
...
popvars <- select(gw_df_test, starts_with("per")) %>% names()
...
region_bbox <- gw_json_export$regionBoundingBox
bbox <- c(region_bbox$lonMin, region_bbox$latMin, region_bbox$lonMax, region_bbox$latMax)

descriptionTitle <- gw_json_export$descriptionTitle

var1_name <- gw_json_export$dataColumns$name[1]
var1_name_index <- match(var1_name, gw_json_csv$schema$fields$name)
var1_title <- gw_json_csv$schema$fields$title[var1_name_index]

# remove objects not needed for RMD file ----
rm(bar_plots, bar_plots_tot, group_vars, gw_csv, pop, pop_total,
   gw_csv_id, gw_json_boxid, pal, var_name, bin_function,
   gw_json_csv, gw_json_export)

# Save for use in rmd file ----
save.image("gw_pop_example.Rdata")
